...
- Does the draft handoff's "next task" match the first `[ ]` in the active plan file (skipping `[~]`-superseded)? If not, resolve before writing — fix the handoff, mark the plan task done/superseded, or document the deferral.
